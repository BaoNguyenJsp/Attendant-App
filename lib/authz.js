...
// catalog = toàn bộ Groups sheet {name,type,scope} (không chỉ nhóm của user) để BQT/BCH quy chiếu
// đúng ngành cha; lọc kết quả xuống các lớp có thật để tên ngành không bao giờ lọt vào scope.
function expandScope(groups, catalog, allClassNames) {
  const byName = new Map((catalog || groups).map((g) => [g.name, g]));
  const set = new Set();
  ...
  const out = [...set];
  return allClassNames.length ? out.filter((c) => allClassNames.includes(c)) : out;
}
...
// Các ngành mà user (BQT) quản lý — dùng cho điểm danh Huynh trưởng.
...
function makeAuthz() {
  ...
  const requireSession = (req, res, next) => {
    ...
    // Nhóm vừa bị sửa → huỷ session để lần vào trang kế tiếp phải đăng nhập lại
    // (lúc đó quyền mới được tính từ sheet). Cờ tự xoá khi họ đăng nhập thành công.
    if (signal.isStale(req.session.email)) {
      req.session = null;
      return send(res, 401, 'Quyền của bạn vừa thay đổi. Vui lòng đăng nhập lại.');
    }
    next();
  };
  ...
  const requireScopeWrite = (req, res, next) => {
    ...
    // FIX: Allow read-only statistical endpoints to bypass the class scope check
    const readOnlyEndpoints = ['getClassAttendanceStats', 'getSummary', 'getAcademicRecord', 'searchByIdNumber'];
    if (readOnlyEndpoints.some(ep => req.path.includes(ep) || (req.body && req.body.action === ep))) {
      return next(); 
    }
  ...
  // Điểm danh Huynh trưởng: admin mọi ngành; BQT chỉ ngành mình quản lý.
